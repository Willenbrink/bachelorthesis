...
A term can be represented as a tree with all symbols $s$ with $\arity (s) = 0$ as leafs and all functions $f(x_{1}, ..., x_{n})$ as internal nodes with the $x_{i}$ as children. Within this tree, every symbol has a position by the \todo{}
...
\begin{enumerate}
  ...
  \item A function $f(x_{1},...,x_{n})$ is unifiable with term $t$ if and only if $t = x$ or $t = f(y_{1},...,y_{n})$ where, for all $i$, $x_{i}$ is unifiable with $y_{i}$.
\end{enumerate}
...
\begin{tabular}{ l|l l l }
  \multirow{2}{4em}{Arguments Query} & $Q(p, x)$ & $Q(p, a)$ & $Q(p, f(t_{1}, ..., t_{n}))$ \\\\
  \hline
...
But what about functions? The variable may be replaced by terms with an arbitrary number of arguments. We must skip the nodes corresponding not only to $g$ but also each argument $x_{1}, ..., x_{n}$ of $g(x_{1},...,x_{n})$. When continuing in the $g$ branch, we must not continue at $slp(M,g)$, but at $slp(M,\ang{g,a})$, as the variable is substituted by $g(a)$.

\begin{defn}
  $skip(N)$ returns the set of nodes obtained by skipping a single term starting at $N$. That is, for a constant $c$ with $\arity(c) = 0$ we return $slp(N,c)$ (which is a direct child of $N$). For a function $f(x_{1},...,x_{n})$, we return the nodes $skip^n(slp(N,f))$. If for all $x_{i}$ $arity(x_{i}) = 0$, $skip^n$ retrieve the nodes $1 + n$ levels below $N$.
\end{defn}
...
\begin{tabular}{ l|l l l }
  \multirow{2}{4em}{Arguments Query} & $Q(N, \angt{x})$ & $Q(N, \angt{a})$ & $Q(N, \angt{f(x_{1}, ..., x_{n})})$ \\\\
  \hline
  $Q = \var$ & $Q(slp(N, *), \angtn )$ & $Q(slp(N,a),\angtn )$ & $Q(slp(N,f), \angt{x_{1}, ..., x_{n}})$\\
  $Q = \ins$ & $\underset{M \in Skip(N)}{\bigcup}Q(M, \angtn )$ & $Q(slp(N,a),\angtn )$ & $Q(slp(N,f), \angt{x_{1}, ..., x_{n}})$\\
  $Q = \gen$ & $Q(slp(N, *), \angtn )$ & \multirow{2}{4em}{$Q(slp(N,a),\angtn )$ $\cup\ Q(slp(N,*), \angtn )$} & \multirow{2}{4em}{$Q(slp(N,f), \angt{x_{1}, ..., x_{n}})$ $\cup\ Q(slp(N,*), \angtn )$}\\\\
  $Q = \unif$ & $\underset{M \in Skip(N)}{\bigcup}Q(M, \angtn )$ & \multirow{2}{4em}{$Q(slp(N,a),\angtn )$ $\cup\ Q(slp(N,*), \angtn )$} & \multirow{2}{4em}{$Q(slp(N,f), \angt{x_{1}, ..., x_{n}})$ $\cup\ Q(slp(N,*), \angtn )$}\\
\end{tabular}
}
\caption{The recursive definition of the queries}
...
%\begin{figure}[h]
  ...
  %\scalebox{.8}{
%\begin{tabular}{ l|l l l }
  %\multirow{2}{4em}{Arguments Query} & $Q(N, \angt{x})$ & $Q(N, \angt{a})$ & $Q(N, \angt{f(x_{i}, ...)})$ \\\\
  %\hline
  %$Q = variants$ & $Q(slp(N, *), \angtn )$ & $Q(slp(N,a),\angtn )$ & $Q(slp(N,f), \angt{x_{i}, ...})$\\\\
  %$Q = instances$ & $\underset{M \in Skip(N)}{\bigcup}Q(M, \angtn )$ & $Q(slp(N,a),\angtn )$ & $Q(slp(N,f), \angt{x_{i}, ...})$\\\\
  %$Q = generalisations$ & $Q(slp(N, *), \angtn )$ & \multirow{2}{4em}{$Q(slp(N,a),\angtn )$ $\cup\ Q(slp(N,*), \angtn )$} & \multirow{2}{4em}{$Q(slp(N,f), \angt{x_{i}, ...})$ $\cup\ Q(slp(N,*), \angtn )$}\\\\\\
  %$Q = unifiables$ & $\underset{M \in Skip(N)}{\bigcup}Q(M, \angtn )$ & \multirow{2}{4em}{$Q(slp(N,a),\angtn )$ $\cup\ Q(slp(N,*), \angtn )$} & \multirow{2}{4em}{$Q(slp(N,f), \angt{x_{i}, ...})$ $\cup\ Q(slp(N,*), \angtn )$}\\\\
%\end{tabular}
%}
%\caption{The recursive definition of the queries}
%\label{discnetqueries}
